...
| Is there such a thing as a single, worldwide CSP account?   |  No. The CSP sales regions are dependent on the location of the (AzureAD) tenant. So a CSP Partner can only sell or manage customers that have a tenant located in the same CSP region as the Partner enrolled as CSP (i.e., the location of the tenant the CSP Partner is using to transact). So, if the customer tenant was created in the US, only a Partner that has a CSP enrollment in the US can establish a reseller relationship with this customer. With regards to Autopilot & Intune, it does not matter where the end user/device is located though, e.g. a device that is used by an employee located in Germany would be able to enroll using the autopilot profile created in the US tenant and, subsequently, would be managed by the Intune service instance in US. The user in Germany would also authenticate in the US AzureAD instance. So if a partner wants to manage customers globally, they need to have a global presence, e.g. multiple CSP enrollments in each of the CSP sales regions they want to sell in. It is also not possible to create user accounts that have access to all CSP tenants; it would translate into 18 user accounts for a CSP Admin Agent that wants to be able to manage all customers around the world. In summary, the location of the user & devices does not matter, the location of the customer tenant does. Cross-border device registration is not the problem; cross-border sales via CSP is. |
