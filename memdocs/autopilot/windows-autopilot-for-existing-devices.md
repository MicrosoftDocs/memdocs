...
- Acquire the installation media for Windows 10 version 1809, or later. Then create a Configuration Manager OS image. For more information, see [Manage OS images](../configmgr/osd/get-started/manage-operating-system-images.md).
...
1. Save one of the profiles to an ANSI-encoded file named **AutopilotConfigurationFile.json**. Save it to a location suitable as a Configuration Manager package source.
...
1. Create a Configuration Manager package that contains this file. It doesn't require a program. For more information, see [Create a package](../configmgr/apps/deploy-use/packages-and-programs.md#create-a-package-and-program).
...
1. On the **Task sequence information** page, specify a name, optionally add a description, and select a boot image. For more information on supported boot image versions, see [Support for Windows 10](../configmgr/core/plan-design/configs/support-for-windows-10.md#windows-10-adk).

1. On the **Install Windows** page, select the Windows 10 **Image package**. Then configure the following settings:
...
1. On the **Configure Network** page, select the option to **Join a workgroup**. This task sequence uses the Windows system preparation tool (sysprep). If the device is joined to a domain, sysprep fails.
...
1. The **Include updates** page selects by default the option to **Do not install any software updates**.
    ...
    > [!TIP]
    > Use offline image servicing to keep the image up to date with the latest Windows 10 quality updates. For more information, see [Apply software updates to an OS image](../configmgr/osd/get-started/manage-operating-system-images.md#BKMK_OSImagesApplyUpdates).
...
- [Windows Autopilot with co-management](../configmgr/comanage/quickstart-autopilot.md)
