...
> [!NOTE]
> Intune will periodically check for new devices in the assigned groups, and then begin the process of assigning profiles to those devices. Due to several different factors involved in the process of AP profile assignment, an estimated time for the assignation can vary from scenario to scenario. These factors can include AAD groups, membership rules, hash of a device, Intune and AP service, and internet connection. The variance between the different scenarios will increase or decrease depending on all the factors and variables involved in that specific scenario. Before deploying a device, ensure that this process has completed. You can check under **Devices** > **Windows** > **Windows enrollment** > **Devices** (under **Windows Autopilot Deployment Program** where you should see the profile status change from "Unassigned" to "Assigning" and finally to "Assigned."
