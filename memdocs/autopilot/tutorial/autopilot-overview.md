...
Windows Autopilot is a technology that configured a Windows device for first time use. Normally this would be when a new device is first delivered to an end-user via an OEM, reseller, or IT organization, but it can also be used to configure an existing devices after a reset.
...
- Automatically joining of devices into Azure Active Directory (via Azure AD join) or Active Directory (via hybrid Azure AD join).
...
- Eliminates the need to create and maintain operating system images by using the image already installed on the device by the OEM.
- Eliminates the need to manage drivers by using the drivers installed on the device by the OEM.
- Simplifies the process of deploying devices to end-users that are remote.
- Allows end-users to run through the whole initial deployment process of the device without IT intervention.
- Eliminates the need for IT to touch the device before delivering to an end-user. Devices can be directly shipped and delivered to the end-user.
...
## Windows Autopilot scenarios pros and cons
...
### When to use Windows Autopilot user-driven
...
### When to use Windows Autopilot for pre-provisioned
...
### When to use Windows Autopilot self-deploying mode
...
### When to use Windows Autopilot for existing devices
