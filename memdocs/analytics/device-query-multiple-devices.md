...
| EntraDeviceId | String | Unique ID generated by Entra as part of Entra registration or join. |
| ManagementName | String | An easily recognizable device name used only in the Intune admin center. Changing this name does not change the device name or the name in the Company Portal. |
...
The device entity shows up in query results, unless specified not to (through something like a project that does not include the Device entity). The device entity and its properties can also be referenced in queries by referencing Device.[insert property].
...
Writing a query that projects multiple Device columns will not run, with the exception of Device and Device.DeviceId.  

For example, the following query will fail: DiskDrive | project Device.DeviceName, Device.DeviceId 
...
For example, the following query will show a red underline under “Device” but can still be run: Cpu | summarize max(Device) by Manufacturer 

When writing queries with a join operator, $left and $right parameters will show a red underline under $left and $right. However, the query will still be able to run and will return results as expected.  

A single query can contain a maximum of 3 join operators. Queries with additional joins will fail. 
