...
1. The Microsoft Endpoint Management service processes data for each device and publishes the results for both individual devices and organizational aggregates in the admin console using [MS Graph APIs](https://docs.microsoft.com/graph/api/resources/intune-device-mgt-conceptual?view=graph-rest-beta). The maximum latency end to end is 25 hours and is gated by the time it takes to do the daily processing of insights and recommendations.
