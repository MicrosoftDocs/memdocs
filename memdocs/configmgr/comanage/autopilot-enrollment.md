...
- A device group in Intune to which you'll assign the co-management settings policy. For more information, see [Add groups to organize users and devices](../../memdocs/intune/fundamentals/groups-add.md).

  You also need to assign the following profiles to the same device group:

  - [Enrollment status page profile](../../memdocs/intune/enrollment/windows-enrollment-status.md), with the option to **Show app and profile configuration progress**
...
- Don't use this process with other policy providers like the [Intune management extension](../../memdocs/intune/apps/intune-management-extension.md), which can cause conflicts. Each provider isn't currently aware of others. Either use the co-management policy for the Configuration Manager provider, or use the Intune management extension provider, not both.
...
1. On the **Assignments** page, select a target _device_ group. For more information, see [Assign user and device profiles in Microsoft Intune](../../memdocs/intune/configuration/device-profile-assign.md).
