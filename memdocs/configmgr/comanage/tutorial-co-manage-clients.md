...
> [!NOTE]
> Since devices are enrolled in the Microsoft Intune service based on its Azure AD device token, and not a user token, only the default [Intune enrollment restriction](../../memdocs/intune/enrollment/enrollment-restrictions-set.md) will apply to the enrollment. 
