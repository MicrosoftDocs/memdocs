...
-   For this scenario  you might need to install the **SQL Server 2012 native client** ([from the SQL Server 2012 Feature Pack](https://www.microsoft.com/en-us/download/details.aspx?id=56041)) on the management point server. This prevents SQL Server connection errors (which are logged in the **mp_getauth.log** on the management point server).  
