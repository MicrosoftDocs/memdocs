...
1. The **If content is unavailable from distribution points in the current boundary group, immediately fallback to a neighbor or the site default** setting has the following options:
   - **Yes**: Allows fallback to occur immediately
...
- **Yes**: When a client is allowed to fall back to a neighbor or the site default boundary group for the software update deployment, fallback occurs immediately. The client doesn't wait to reach the fallback time (in minutes) defined by the [Boundary Group relationship](../../../../servers/deploy/configure/boundary-group-procedures.md#bkmk_bg-fallback).
