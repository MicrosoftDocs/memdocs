...
After you have removed a passive site server role, the **SMS Provider** role is not automatically removed and you cannot remove the server from Configuration Manager because it still has a role. You need to start Configuration Manager Setup from the site server and select **Site Maintenance** to remove the **SMS Provider** role. 
