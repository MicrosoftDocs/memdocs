...
After updating to Configuration Manager current branch, version 2203, the registration process fails for clients using public key infrastructure (PKI) for client authentication, but are unable to authenticate against the domain. This affects the following scenarios:
