...
|The admins install the Endpoint Protection site system role on one site system server only, at the top of the Woodgrove Bank hierarchy.|For more information about how to install the Endpoint Protection site system role, see "Prerequisites" in [Configure Endpoint Protection](/sccm/protect/deploy-use/endpoint-protection-configure).|  
