...
1. Be sure to have enough Intune licenses to cover all your users managed by Basic Mobility and Security.
2. Review the [device security policies]( https://docs.microsoft.com/microsoft-365/admin/basic-mobility-security/set-up#set-up-mobile-device-management) in the Office 365 Security & Compliance portal and delete any that you no longer need. Deleting unneeded policies reduces the number of recommendations created by the migration tool so you have fewer to review.
3. Review the [membership of groups]( https://docs.microsoft.com/microsoft-365/admin/basic-mobility-security/create-device-security-policies#step-3-deploy-a-policy-to-your-organization) currently assigned device security policies. If these groups contain users that are already licensed for Intune, you should create new groups to separate Intune-licensed users from groups without Intune licenses. Only assign device security policies to those users who aren't assigned Intune licenses.
...
5. Before migration, don’t assign Intune licenses to users whose devices are managed by Basic Mobility and Security. Don't assign Intune licenses to enable Intune app protection without enrollment (MAM-WE). Only assign Intune licenses to users after the policy migration step is complete. The license assignment controls the migration of devices from Basic Mobility and Security to Intune. Before the first device is moved, make sure that sufficient Intune policies have been created to replace Basic Mobility and Security.   
...
After you review and possibly change the recommendations, you can assign the policies to Intune groups. The new policies will go into effect after you assign Intune licenses to the users and the next [refresh cycle](../configuration/device-profile-troubleshoot.md#how-long-does-it-take-for-devices-to-get-a-policy-profile-or-app-after-they-are-assigned) occurs.

> [!NOTE]
> For the Windows operating system, only Windows 10 desktop devices will have policy migrated for them. Other versions of Windows and Windows Phone won't have policy migrated for them. For more information, see the [Policy mapping for Intune and Basic Mobility and Security](). 
...
2. Open the [Migration evaluation tool]() > choose **Start**. It will take a few minutes to complete the evaluation.
    >[!NOTE]
...
5.	Select **Details** to review the suggested recommendation.
    ...
    > [!NOTE]
    > If you make any changes to the migrated email profiles or fail to assign them to recommended groups, users may be asked to re-enter their username and password to access email on their devices when the device migrates to Intune (See below). 
...
9. Assign Intune [licenses](https://docs.microsoft.com/azure/active-directory/fundamentals/license-users-groups.md) to the users. The new policies will go into effect after the next [refresh cycle](../configuration/device-profile-troubleshoot.md#how-long-does-it-take-for-devices-to-get-a-policy-profile-or-app-after-they-are-assigned).
