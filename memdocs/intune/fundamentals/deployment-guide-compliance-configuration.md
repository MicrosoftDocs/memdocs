...
- Your devices [are enrolled](../enrollment/device-enrollment.md) in Intune. This requirement includes devices that are co-managed, or hybrid Azure Active Directory (Azure AD) joined devices.
...
- In the [Endpoint Manager admin center](https://go.microsoft.com/fwlink/?linkid=2109431):
  ...
  - Use [ADMX templates](../configuration/administrative-templates-windows.md) to configure settings for Microsoft Office, Microsoft Edge version 77 and later, and Windows. You don't install anything, as the templates are automatically built in.
  ...
  - Use [guided scenarios](guided-scenarios-overview.md) to complete a specific task, end-to-end. For example, you can secure Office apps, deploy Edge, and setup a cloud-managed PC.

    If you're new to Intune, and looking for step-by-step guidance, then guided scenarios may be a good option.
...
1. Create [compliance policies](../protect/device-compliance-get-started.md) that focus on security. This initial set of policies is your baseline, or minimum set of security rules that all devices must have. In [create a priority list](#create-a-priority-list) (in this article), you created a list of policies devices must have. To get started, use this list. Also consider:
    ...
    - In the [Endpoint Manager admin center](https://go.microsoft.com/fwlink/?linkid=2109431), use [Security Baselines](../protect/security-baselines.md). These security settings are pre-configured by security experts. You create a template, and assign the template to your groups. You can also change any setting to a different value, if you want.

      If you're brand new to the cloud, or want automatically configured Microsoft-recommended security settings, then use [Security Baselines](../protect/security-baselines.md).
    ...
    - When you create your policies, look for patterns. There may be settings that apply to only Windows devices, or only Android devices, or settings that apply to all platforms. The following list includes some of the commonly configured compliance settings for each platform:
