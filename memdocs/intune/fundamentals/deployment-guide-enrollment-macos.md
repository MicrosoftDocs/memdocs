...
Company Portal for macOS detects the installation of the management profile and automatically registers the device, unless it is manually closed by the user. If the user closes the app, they must reopen it to complete device registration. If you're using dynamic groups, which rely on device registration, it's important for users to complete device registration. If you're using Conditional Access (CA) policies, no action is required because any CA-protected app users try to sign into will prompt them to return to Company Portal to complete device registration. 
