...
- For iOS users: Users with notifications enabled will receive a prompt to return to Company Portal for iOS. If they disabled notifications, they will not be alerted but still need to return to Company Portal to complete registration.

- For macOS devices: Company Portal for macOS will detect the installation of the management profile and automatically register the device unless it's manually closed by the user. If the users closes the app, they must reopen it to complete registration.
...
- [Enrollment guide: Enroll iOS and iPadOS devices in Microsoft Intune](../fundamentals/deployment-guide-enrollment-ios.md#user-and-device-enrollment-end-user-tasks)  
