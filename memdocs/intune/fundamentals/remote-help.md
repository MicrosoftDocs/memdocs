...
- **Use Remote Help with enrolled and unenrolled devices**: Remote Help is supported on enrolled devices that also need to be Entra registered devices. To allow Remote Help on devices that aren't enrolled in intune, you must turn on this setting that is disabled by default.
