...
Before you begin, complete these prerequisites to enable iOS device management in Intune. For more detailed information about how to set up, onboard, or move to Intune, see the [Intune setup deployment guide](deployment-guide-intune-setup.md).          
...
|[Add and assign Mobile Threat Defense (MTD) apps](../protect/mtd-apps-ios-app-configuration-policy-add-assign.md)|Add and deploy mobile threat defense (MTD) apps in Intune. These apps work with your device compliance and app protection policies to identify and help remediate device threats. |  Android Enterprise, Android device administrator |  
...
|[Use PKCS certificates with Intune](../protect/certificates-pfx-configure.md)|Configure required infrastructure (such as on-premises certificate connectors), export a PKCS certificate, and add the certificate to an Intune device configuration profile. |Android device administrator, Android Enterprise| 
|[Use imported PKCS certificates with Intune](../protect/certificates-imported-pfx-configure.md)|Set up imported PKCS certificates, which enable you to [set up and use S/MIME to encrypt email](../protect/certificates-s-mime-encryption-sign.md). **Does this make sense to call out S/MIME encryption here? The linked article mentions it as if it's the main (maybe only?) benefit for imported cert. But it seems that non-imported certs can be used for that too, so I worry that this task would mislead people in to thinking it's the BEST way to set up S/MIME. Thoughts?** |Android device administrator, Android Enterprise|
...
|[Add built-in apps](../apps/apps-add-built-in.md) | Add built-in apps to Intune and assign to groups. | Android device administrator, Android Enterprise| 
