...
- Windows - Supported on Windows 8.1 and later
    > [!Note] 
    > Windows attempts to show the user a message with the following text: "Your device administrator has scheduled a reboot." This message is show at same time as the 5 minute restart counter is started. Restart of Windows devices immediately requires push notifications via Windows Notification Services (WNS). For more information on WNS, see [Network Endpoint Requirements](/mem/intune/fundamentals/intune-endpoints.md#windows-push-notification-services-wns).
