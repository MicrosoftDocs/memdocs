...
### Can I control which messages appear first? 
You can schedule messages to arrive at different times on a device by selecting a unique delivery window for each message. If you schedule messages to arrive within the same time period on the same device, the messages will arrive in random order.  
