...
Windows location services need to enabled during Windows Out of Box Experience(OOBE), by the user, or using policy [Privacy > LetAppsAccessLocation](/windows/client-management/mdm/policy-csp-privacy#privacy-letappsaccesslocation) for them to be able to locate the device.
