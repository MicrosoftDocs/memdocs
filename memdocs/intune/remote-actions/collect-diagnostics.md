...
> [!IMPORTANT]
> For Android devices, if the Company Portal isn't signed in by the user, logs will not be available for download in the Intune portal.  Please note that diagnostic uploads exceeding 50 diagnostics or 4MB in diagnostic data cannot be downloaded directly from the Intune portal. For access to these larger diagnostic uploads, reach out to Microsoft Intune support for assistance. 
