...
In Intune, you can use a **BIOS configuration** device configuration policy to enable or disable BIOS features and settings.
...
6. In **Configuration settings**, configure the following settings:
    ...
    - **Disable per-device BIOS password protection**: This setting manages the password that protects the BIOS configuration on the device. Your options:
      ...
      - **Not configured**: ??Is this an option??
    ...
    - **Configuration file**: Upload the configuration file generated with your OEM tool. To upload the file, select **Browse** and then select the file.
...
8. In **Assignments**, select the users or user group that will receive your profile. For more information on assigning profiles, go to [Assign user and device profiles](device-profile-assign.md).
     ...
     ??Should this policy be assigned to devices??
...
Intune has two features that can manage the BIOS settings on Windows devices: **BIOS configuration** and **Device Firmware Configuration Interface (DFCI)**.
