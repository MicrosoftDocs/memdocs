...
title: Create a policy using the settings catalog in Microsoft Intune
description: Use the settings catalog with Windows, iOS, and macOS devices for Microsoft Office apps, Microsoft Edge, and more. Use Microsoft Copilot to get What if analysis and learn about settings.
...
The settings catalog lists all the settings you can configure and all in one place. This feature simplifies how you create a policy and how you see all available settings. For example, you can use the settings catalog to create a BitLocker policy with all BitLocker settings.
...
To manage and secure devices in your organization, use the settings catalog as part of your mobile device management (MDM) solution. More settings are continually being added to the settings catalog. For a list of the settings, see the [IntunePMFiles / DeviceConfig GitHub repository](https://github.com/IntunePMFiles/DeviceConfig).

This feature applies to:

- **iOS/iPadOS**

  Includes device settings that are directly generated from Apple Profile-Specific Payload Keys. More settings and keys are continually being added. To learn more about profile-specific payload keysFor more information, see [Profile-Specific Payload Keys](https://developer.apple.com/documentation/devicemanagement/profile-specific_payload_keys) on Apple's website.

  Apple's declarative device management (DDM) is built into the settings catalog. When you configure settings from the settings catalog on iOS/iPadOS 15+ devices enrolled using [User Enrollment](../enrollment/ios-user-enrollment.md), you're automatically using DDM. If DDM doesn't work for any reason, these devices use Apple's standard MDM protocol. All other iOS/iPadOS devices continue to use Apple's standard MDM protocol.
...
## Find and learn about each setting
...
- To find specific settings in your policy, you can use **Add settings** > **Search**. You can search by category, such as `browser`; search for a keyword, such as `office` or `google`; or search for specific settings.
...
Select **Duplicate** to create a copy of an existing profile. Duplicating is useful when you need a profile that's similar to but distinct from the original.
...
When you create a settings catalog policy, you can export the policy to a `.json` file. You can then import this file to create a new policy. This feature is useful if you want to create a policy that's similar to an existing policy. For example, export a policy, import it to create a new policy, and then make changes to the new policy:
...
If you use Copilot, you can use some built-in prompts to get more information on existing policies, including their impact.
...
1. In the [Intune admin center](https://go.microsoft.com/fwlink/?linkid=2109431), select **Devices** > **Configuration**. In the list, select the policy you created by using the settings catalog. The **Profile type** column shows **Settings Catalog**:
...
2. When you select the policy, device status is displayed. It shows a summary of your policy state and the policy properties. You can also change or update your policy in the **Configuration settings** section:
...
3. Select **View report**. The report shows detailed information, including the device name, the policy status, and more. You can also filter on the deployment status, and **Export** the report to a `.csv` file:
    ...
    :::image type="content" source="./media/settings-catalog/settings-catalog-policy-view-report.png" alt-text="Screenshot that shows where detailed report information in Microsoft Intune and Intune admin center, including device name, policy status, and more." lightbox="./media/settings-catalog/settings-catalog-policy-view-report.png":::

4. You can also look at the states of each setting by using **per-setting status**. This status shows the total number of devices affected by each setting in the policy.
...
> [!TIP]
> [Intune reports](../fundamentals/reports.md) is a great resource that describes all the reporting features you can use. For details, see [Intune reports](../fundamentals/reports.md).
...
1. In the [Intune admin center](https://go.microsoft.com/fwlink/?linkid=2109431), select **Devices** > **Configuration**. In the policy list, select the policy you want to evaluate by using Copilot.
...
3. In the prompt, there are various you can select. You can also select the prompt guide and select from an existing list of prompts:
...
When you create the policy, you can choose from two policy types: **Settings catalog** and **Templates**:

:::image type="content" source="./media/settings-catalog/select-windows-policy-type.png" alt-text="Screenshot that shows where you select settings catalog or templates in Microsoft Intune and Intune admin center when you create a Windows or macOS policy.":::

**Templates** includes a logical group of settings, such as kiosk, VPN, Wi-Fi, and more. Use this option if you want to use these groupings to configure your settings.
...
Some settings include a `(User)` tag or `(Device)` tag in the setting name, such as `Allow EAP Cert SSO (User)` or `Grouping (Device)`. When you see these tags, the policy only affects the user scope or the device scope.
...
When a device checks in to Intune, the device presents a `deviceID`. The device might or might not present a `userID`, depending on the check-in timing and whether a user is signed in.
