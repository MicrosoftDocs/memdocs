...
- **Password sync** (Kerberos only): Choose **Yes** to sync your users' local passwords to Active Directory. When set to **Not configured** (default), Intune doesn't change or update this setting. By default, the OS might disable password sync to Active Directory. Use this setting as an alternative or backup to SSO. This setting doesn't work if users are signed in with an Apple mobile account.
