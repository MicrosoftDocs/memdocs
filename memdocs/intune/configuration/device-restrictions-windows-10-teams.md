...
- **Welcome screen background image URL**: Enter the URL of a .png image that you want as the background during user sessions and on the **Welcome** screen on Windows 10 Team devices. The image must be in PNG format, and the URL must begin with `https://`.
