...
- ADMX settings that are built into Windows (located in the `C:\Windows\PolicyDefinitions` folder) are enabled through configuration service providers (CSPs). Don't import these built-in settings using the import ADMX feature with the intent to configure them. They should only be uploaded if they are a required parent namespace of another file. Instead, use the [settings catalog](settings-catalog.md) or a [custom profile](custom-settings-configure.md).
