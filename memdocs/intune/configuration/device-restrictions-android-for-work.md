    ...
    - **Multi-app**: Users can access a limited set of apps on the device. When the device starts, only the apps you add start. You can also add some web links that users can open. When the policy is applied, users see icons for the allowed apps on the home screen.
      ...
      - **Sign-in screen**: **Enable** shows a sign-in screen on the Managed Home Screen. When set to **Not configured** (default), Intune doesn't change or update this setting. This sign-in screen and related settings are intended for use on dedicated devices enrolled with AAD Shared device mode.
        ...
        - **Require user to set a PIN for sign-in session**: When set to **Enable**, the user must set a PIN for their sign-in session. When set to **Not configured** (default), the user isn't required to set a PIN. This setting must be enabled to show the sub-settings.
          - **Choose complexity of PIN for sign-in session**: Choose the complexity of the session PIN. Options include **Not configured**, **Simple**, or **Complex**. Simple only requires numbers and has no restrictions on repeating (444) or or ordered (123, 321, 246) sequences. Complex allows users to create a PIN with alphanumeric characters and requires no repeating (444) or ordered (123, 321, 246) sequences. See additional configurations for Session Pin available in app config [here](https://docs.microsoft.com/en-us/mem/intune/apps/app-configuration-managed-home-screen-app).
