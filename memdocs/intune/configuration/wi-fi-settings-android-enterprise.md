...
- **Connect automatically**: **Enable** automatically connects to your Wi-Fi network when devices are in range. Select **Disable** to prevent or block this automatic connection. Note that the device will not automatically connect to this Wi-Fi network if it's already connected to another preferred Wi-Fi connection. Disconnect the device from any existing Wi-Fi connections if it fails to connect automatically despite this seting being enabled.
