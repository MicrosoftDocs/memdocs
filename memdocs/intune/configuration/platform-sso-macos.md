...
- Get the benefits of entra ID join, which allows any org user to sign into the device 
...
Using Intune, you add the correct version of the Company Portal app, make it a required app, and then deploy the app to your macOS devices. For the specific steps, go to:
...
If these are **existing devices** that are already enrolled, then you can assign the policies to the devices. The next time the devices sync or check-in with the Intune services, they receive the platform SSO policy settings you created.
