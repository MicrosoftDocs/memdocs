...
- **Kerberos principal name**: Intune looks for this attribute for each user in Azure AD. Intune then populates the respective field (such as UPN) before generating the XML that gets installed on devices. Your options:
  ...
  - **Not configured**: Intune doesn't change or update this setting. By default, the OS may prompt users for a Kerberos principal name during profile installation onto the device because a principal name is required for MDM installation of the profile.
