...
For example, schools have devices that are typically used by many students. The school Intune administrator can turn on the Shared PC feature to allow one user at a time. When enabled:

- Students can't switch between different signed-in accounts on the device.
- When the student signs out, you can choose to remove all user-specific settings.
