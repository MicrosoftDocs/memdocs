...
Admins can browse, deploy, and monitor Microsoft Store applications inside Intune. Upon deployment, Intune automatically keeps the apps up to date when a new version becomes available. The Microsoft Store supports UWP apps, PWAs, desktop apps packaged in *.msix*, and now Win32 apps packaged in *.exe* or *.msi* installers.  
...
> - You can install and uninstall required app deployments
...
2. In the search bar, type the name of the app that you want to find. You can only search by app name. A list of apps are displayed.
    ...
    > - The app is a Microsoft Store for Business app, which is not public in the consumer store
...
3. Choose the app that you want to deploy and click **Select**. 
   The app information is presented with the selected app’s metadata. Specific fields are pre-populated.
    ...
    |---|---|---|
    |     Name    | Enter the name of the app as it appears in the Company Portal. Make sure all app names that you use are unique. If the same app name exists twice, only one of the apps appears in the company portal.   |     Required    |
    ...
    |     Package   Identifier     | The app’s unique ID in the Microsoft Store.          |     N/A   Pre-filled    |
    |     Install   behavior    | The install behavior of the app. If the app to be installed has the option of either **System** or **User** install behaviors, you must ensure that the installation works on devices as expected.     |     Admin must select **System** or **User**     |
...
3. After you select your groups, choose whether to set **End user notifications**, **Availability**, and **Installation deadline**. 
4. If you don't want the app assignment to affect groups of users, select **Included** under the **MODE** column. In the **Edit assignment** pane, change the **mode** value from **Included** to **Excluded**. Select **OK** to close the **Edit assignment** pane.
...
Apps that are deployed from the Microsoft Store are automatically kept up to date to the latest version of the app. For this feature to work properly for UWP apps, the **group policy for store auto-update** should not be disabled.
...
|     Store\Turn off Automatic Download and   Install of updates    | Turn on Automatic Download and Install of updates          |
|     Desktop App Installer\Enable App Installer   Microsoft Store Source    | Enable App Installer Microsoft Store Source          |
|     Desktop App Installer\Enable App Installer    | Enable App Installer          |
