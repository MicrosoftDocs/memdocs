...
> [!TIP]
> Some of these items can be configured using a device configuration profile. For more information on the settings you can configure, see [Android Enterprise device experience settings](../configuration/device-restrictions-android-for-work.md#device-experience).
...
| Application order enabled | bool | FALSE | Turning this setting to True enables the ability to set the order of applications, weblinks, and folders   on the Managed Home Screen. Once enabled, set the ordering   with app_order. | ❌ |
