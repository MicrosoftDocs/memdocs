...
| Device conditions  |       Play integrity verdict evaluation type  |          Check strong integrity  |          Android  |          Hardware backed attestation enhances the existing device integrity check service check by leveraging a evaluation type called [Strong Integrity](https://developer.android.com/google/play/integrity/verdict#device-integrity-field), providing a more robust root detection in response to newer types of rooting tools and methods that cannot always be reliably detected by a software only solution.<p> Hardware backed attestation leverages a hardware-based component which shipped with devices installed with Android 8.1 and later. Devices that were upgraded from an older version of Android to Android 8.1 are unlikely to have the hardware-based components necessary for strong integrity evaluation. While this setting should be widely supported starting with devices that shipped with Android 8.1, Microsoft strongly recommends testing devices individually before enabling this policy setting broadly.</p>  |
