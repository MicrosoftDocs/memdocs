...
7. On the **Platform settings** page, configure the restrictions for your selected platform. Your options:       
    ...
    - **Allow min/max range** (Android, Windows, iOS/iPadOS only): Enter the minimum and maximum OS versions allowed to enroll. Supported version formats include:  
        ...
        - iOS/iPadOS supports major.minor.rev.  
          > [!TIP] This setting isn't applicable to Apple devices that enroll with the Device Enrollment Program, Apple School Manager, or the Apple Configurator app. Althought Intune doesn't block ADE enrollments that authenticate with Company Portal, not meeting OS requirements impacts the registration of these devices because they can't create the Azure AD Device record used to evaluate Conditional Access policies. You can tell that this is the case if device users receive an error message that says "Couldn't map device record with a user" after they sign in to Company Portal.     
