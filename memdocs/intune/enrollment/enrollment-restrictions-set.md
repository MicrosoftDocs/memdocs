...
### Enrollment failures report  
Use the enrollment failures report to view enrollment failures for all users or for select users. This report shows every failed enrollment attempt along with the date it occurred, reason for failure, OS, OS version, username, and enrollment method.  
...
3. Select a row for more details about the failure and recommended remediation steps.  

![Example image of the Device enrollment page, Enrollment failures report, showing the enrollment failure details for a selected row.](https://user-images.githubusercontent.com/19563363/146618606-2ca8042e-4978-4cc9-ac29-c209223a76af.png)
...
### Device enrollment page
The device enrollment page shows the enrollment policies (both enrollment restriction and enrollment status page policies) applied to a device when it was first enrolled in Microsoft Intune. Use the report on this page to help you pinpoint the source of the failure, such as an unexpected policy, target, or filter. Report data is available for iOS/iPadOS, macOS and Windows devices and includes: 
...
>[!NOTE]
>You must enable filters to view the device enrollment page. The data on this page is only available for devices enrolled after the Microsoft Intune 2112 service release. No results are available for devices enrolled prior to that release.
