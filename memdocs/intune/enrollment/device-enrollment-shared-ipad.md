...
## Configure temporary sessions on Shared iPads
 ...
 Temporary sessions allow users to sign in as a guest, and doesn't require them to enter a Managed Apple ID or password. All of their data, including browsing history, is deleted when the user signs out of the session. Temporary sessions are allowed by default on Shared iPads. 
 
 In iPadOS 13.4 or later, users can initiate a [temporary session](https://support.apple.com/guide/mdm/mdm6c592d817/web) by tapping **Guest** on the sign-in screen. You can configure temporary sessions in an iOS device restrictions profile in the admin center. For more information, see [Shared iPad - Automated device enrollment (supervised)](../configuration/device-restrictions-ios.md#settings-apply-to-automated-device-enrollment-supervised-10). 
...
- **Passcode complexity cannot be managed on Shared iPad:** Shared iPad passcodes are required to be made up of eight alphanumeric characters and can't be changed in Apple Business Manager. The passcode complexity and length settings available in device configuration profile do not apply to Shared iPads. The MDM administrator can set the grace period, which specifies the number of minutes a user has to unlock the iPad without a passcode.  
