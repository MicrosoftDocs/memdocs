...
* There is a known issue with web-based enrollment that prevents the Company Portal app from recognizing enrolled devices. This happens when an enrolled device doesn't have the SSO extension policy. When a user tries to sign in to Company Portal for iOS on a device that doesn't have the SSO extension policy, Company Portal is unable to determine that the device has been enrolled. We are actively working to resolve this issue. We recommend deploying the SSO extension policy for enrolling devices. As a temporary workaround, you can deploy a web clip for the web version of Company Portal, as described under [Best practices for web enrollment](web-based-device-enrollment-ios.md#best-practices).    
