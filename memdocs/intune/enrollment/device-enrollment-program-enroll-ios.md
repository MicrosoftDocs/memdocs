...
| **Device to Device Migration** | Give the user the option to migrate data from an old device to this device. The option "Transfer Directly from iPhone" isn't available for ADE devices running iOS 13 and later, so this option won't appear on those devices.  
