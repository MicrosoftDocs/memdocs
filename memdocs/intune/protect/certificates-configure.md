...
| PKCS certificate           | Deploys a template for a certificate request to users and devices. The provisioned certificate is always associated to a user and each of the user’s devices receives a unique certificate. <br><br>When deployed to a device that lacks a user affinity, the certificate isn’t provisioned.    |
