---
title: Use Microsoft Tunnel VPN with devices that don't enroll with Microsoft Intune
description: Add support for Mobile Application Management (MAM) to the Microsoft Tunnel Gateway, a VPN server that runs on Linux. Tunnel support for MAM expands access to your organizational resources for devices that can't or haven't enrolled with Microsoft Intune
keywords:
author: brenduns
ms.author: brenduns
manager: dougeby
...
# Microsoft Tunnel for Mobile Application Management (public preview)

In public preview, you can use Microsoft Tunnel VPN Gateway with unenrolled devices to support Mobile Application Management (MAM) scenarios. With support for MAM, your unenrolled devices can use Tunnel to securely connect to your organization allowing users and apps safe access to your organizational data.
...
- App configuration policy for Microsoft Edge. This policy configures Microsoft Edge on the device to connect to Microsoft Tunnel.
...
Users of devices that aren't enrolled  with Intune must install the following apps on their Android device before they can use the Tunnel for MAM scenario. These apps can all be manually installed the Google Play store:

1. **Microsoft Defender** - Get it from [Microsoft Defender - Apps on Google Play](https://play.google.com/store/apps/details?id=com.microsoft.scmx&hl=en_US&gl=US&pli=1). Microsoft Defender includes the tunnel client app that the device uses to connect to Microsoft Tunnel.
...
## Configure policies to support using MAM with Tunnel
...
Create an App configuration policy for Microsoft Edge. This policy configures Microsoft Edge on the device to connect to Microsoft Tunnel when a device user signs Microsoft Edge on their device with an organizational account.
...
2. On the *Basics* tab:
   ...
   :::image type="content" source="./media/microsoft-tunnel-mam/public-apps-edge.png" alt-text="Screen shot of configuring an app configuration policy with Microsoft Edge as a public app.":::
...
2. On the *Basics* tab:
   ...
   :::image type="content" source="./media/Microsoft-tunnel-mam/public-apps-defender.png" alt-text="Screen shot of configuring an app configuration policy with Microsoft Defender Endpoint as a public app.":::
...
3. On the *Settings* tab, skip the *General configuration settings* category, which isn't used for this policy. For the *Microsoft Tunnel settings* category, make the following configurations:
   ...
   Optionally, configure settings for *Proxy* to meet your requirements, and when ready, select **Next** to continue.
...
Create an app protection policy to automatically start the Microsoft Tunnel connection when the device tries to connect to corporate resources.
...
3. On the *Apps* tab, click **Select public apps**, select **Microsoft Edge**, and then click **Select**.
   ...
   :::image type="content" source="./media/Microsoft-tunnel-mam/app-protection-edge.png" alt-text="Screen shot of configuring an app protection policy with Microsoft Edge as a public app.":::
...
4. On the *Data protection* tab, scroll to the bottom and set *Start Microsoft Tunnel connection on app-launch* to **Yes**, and then select **Next**.
   ...
   :::image type="content" source="./media/Microsoft-tunnel-mam/app-protection-data-protection-tab.png" alt-text="Screen shot of configuring an app protection policy setting for using Tunnel on app-launch.":::
...
Intune doesn't support a method to deploy trusted certificates to unenrolled devices. Without a trusted certificate for internal endpoints that are protected by TLS or SSL certificates from internal certificate authorities, Microsoft Edge reports that the connection isn’t private or is untrusted, and SSL or https connections aren't available. Users can ignore this warning and connect to the endpoint.
