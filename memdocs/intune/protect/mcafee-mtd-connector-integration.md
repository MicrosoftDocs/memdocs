...
The following steps are done in the Trellix console and will enable a connection to the Trellix service for Intune enrolled devices (using device compliance) and unenrolled devices (using app protection policies).
...
- Trellix Mobile Security syncs with Microsoft Entra ID Enrollment Group membership to populate its device's database.

- Allow Trellix Mobile Security admin console to use Microsoft Entra ID Single Sign On (SSO).

- Allow the Trellix Mobile Security app to sign in using Microsoft Entra ID SSO.

For more information about consent and Microsoft Entra ID applications, see [Request the permissions from a directory admin](/azure/active-directory/develop/v2-permissions-and-consent#request-the-permissions-from-a-directory-admin) in the Microsoft Entra ID article *Permissions and consent in the Microsoft Entra ID v2.0 endpoint*.

## To set up Trellix Mobile Security integration

1. Go to Trellix console and sign in with your credentials. To perform the Trellix Mobile Security integration setup process, you must sign in with an Microsoft Entra ID user who has the Global Administrator role. This one-time setup operation uses the Global Administrator rights to grant permission in your organization for the Trellix Mobile Security apps to communicate with Intune.
...
5. After you set Microsoft Intune as the MDM service, the **Microsoft Intune Configuration** window pops up, choose the **Add Microsoft Entra ID** for each option: **Trellix console**, **Trellix Mobile Security iOS and Android apps** to authorize Trellix Mobile Security to communicate with Intune and Azure AD through Azure AD Single Sign-On.
...
6. Choose **Accept** to authorize the Trellix Mobile Security app to communicate with Intune and Azure Active Directory.

7. After you add the console and Trellix Mobile Security iOS and Android apps to Azure AD, add the Azure AD security groups. This addition allows Trellix Mobile Security to synchronize the Azure AD security group with its service.
