...
5. In **Configuration settings**, select **Add settings** > Expand **Declarative Device Management** > **Software Update**.
    ...
    - **Target Local Date Time**: Select the local date time value that specifies when to force the installation of the software update.
      ...
      > [!IMPORTANT]
      > If you create a policy using this setting before the January 2024 release, then this setting shows `Invalid Date` values. The updates are still scheduled correctly and use the values you originally configured, even though the message is shown.
      >
      > To configure a new date and time, you can delete the `Invalid Date` values, and select a new date and time using the date time picker. Or, you can create a new policy. If you create a new policy, to avoid conflicts, remove the values in the original policy.
