...
- Only devices that need the update receive expedited update:
  - Windows Update doesn’t try to expedite the update for devices that have a revision that’s equal to or greater than the update version.
  ...
  > [!IMPORTANT]
  > In some scenarios, Windows Update can install an update that is more recent than the update you specify in expedite update policy. For more information about this scenario, see [About installing the latest applicable update](#identify-the-lastest-applicable-update), later in this article.
...
The following policy settings on a device should be configured as follows. Conflicting values can block installation of expedited updates. For more information about these settings, see [Policy CSP – Update](/windows/client-management/mdm/policy-csp-update).
...
| **RequireUpdateApproval** - This policy isn't supported for desktop devices. Don’t configure this policy and instead use the policy *Update CSP deferrals* or target version to manage which updates are offered and when they're offered. </br></br> Set to: </br> - **Not configured**  |  *Not applicable*     |
...
2. Select **Devices** > **Windows** > **Windows 10 quality updates (preview)** > **Create profile**.
   ...
   :::image type="content" source="./media/windows-10-expedite-updates/create-quality-udpate-profile.png" alt-text="Screen capture of the Create profile UI":::
...
5. In **Settings**, configure **Number of days to wait before forced reboot**. For this setting, select how soon after installing the update a device will automatically restart to complete the update installation. You can select from zero to two days. The automatic restart is canceled if a device manually restarts before the deadline. If an update doesn’t require a restart, this setting isn’t enforced.
   ...
   > [!IMPORTANT]
   > This experience can impact user productivity. Consider using it for those devices or updates that must complete (restart the device) as soon as possible.
...
5. Now consider the following results for *Test-1* and *Test-2*, based on when each is turned back on:
   ...
   - **Test-1** - On March 12th, *Test-1* is powered back on, connects to the network, and checks in with Intune and other services like Windows Update:
   ...
   - **Test-2** - On March 20th, *Test-2* is powered back on, connects to the network, and checks in with Intune and other services like Windows Update:

     1. Windows Update determines that *Test-2* still needs to expedite the update installation, per policy.
     2. Windows Update determines that *Test-2* still needs the update to be expedited, per policy.

     **Result**: With the deferral policy for the March update having expired, *Test-2* installs the March update, skipping over the February update and installing a later update than is specified in policy.
