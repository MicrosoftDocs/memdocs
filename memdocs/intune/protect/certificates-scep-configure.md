...
- **NDES server role** – You must configure a Network Device Enrollment Service (NDES) server role on Windows Server 2012 R2 or later. In a later section of this article, we guide you through [installing NDES](#set-up-ndes).
  ...
  - We recommend you don’t use NDES that's installed on the server that hosts the Enterprise CA. While use of NDES thats installed on an Enterprise CA is supported, this configuration represents a security risk when the CA services internet requests.  
