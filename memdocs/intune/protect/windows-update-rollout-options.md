...
Use rollout options in Microsoft Intune policies for *Feature updates for Windows 10 and later*. Rollout options are schedule configurations you can use to control when Windows Update offers an update to devices that receive your policies.
...
In addition to configuring a gradual rollout, you can also configure [intelligent optimization](#use-intelligent-optimized-group-distribution). With intelligent optimization, the Windows Deployment service makes the update available to devices considered most likely to reveal possible issues before other devices. When using optimization, early success builds confidence that the larger population of your devices will receive the update successfully as well.
...
- **Final group availability** – Configure the last day that Windows Update makes the update available to what will be the final offer group. The last offer group includes all remaining devices that haven’t already received the offer.
...
- Windows Update assigns targeted devices to the groups randomly. When the number of devices doesn’t fit neatly into the number of groups, extra devices are added to the last group.
...
## Use Intelligent optimized group distribution

You can use *intelligent optimized group distribution* to aid the Windows Deployment service in optimizing the rollout of an update to your devices.

When enabled, the Windows Deployment service evaluates the devices that are targeted by the policy to find the smallest set of those devices that exhibit or include the greatest range of characteristics that can affect a successful update. Characteristics include drivers, installed applications, hardware components like BIOS/UEFI, RAM, and more.
...
- With this optimization, after seeing successful results for that first group, you can have strong confidence that the rollout will go smoothly for each additional group.
- If there are issues with the first group, you can halt the deployment before additional devices receive the offer.

>[!IMPORTANT]  
> To optimize data, the Windows Deployment service might need to process the data in a different geographical region than where your tenant resides. If your data must remain the same geographical region as your tenant, do not enable optimization.
