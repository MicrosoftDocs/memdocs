...
1. For **SCEP Server URLS**, paste the SCEP URI. It's important to leave the string `{{CloudPKIFQDN}}` as-is. Intune replaces this placeholder string with the appropriate FQDN when the profile is delivered to the device. The FQDN will be within the *.manage.microsoft.com namespace, a core Intune Endpoint listed in the following artice: [Network Endpoints for Microsoft Intune](../fundamentals/intune-endpoints.md)
