...
End users can retrieve their personal recovery key (FileVault key) using the iOS Company Portal app, the Android Company Portal app, or through the Android Intune app. The device that has the personal recovery key must be enrolled with Intune and encrypted with FileVault through Intune. Using the iOS Company Portal app, the end user can open a web page that includes the FileVault personal recovery key. Using either the Android Company Portal app or the Android Intune app, the end user can see the **FileVault** recovery key needed to access their Mac devices by selecting **Devices** > *the encrypted and enrolled macOS device* > **Get recovery key**. The browser window will open the Web Company Portal and display the recovery key. You can also retrieve the recovery key directly from the Intune Web Company Portal by selecting **Devices** > *the encrypted and enrolled macOS device* > **Get recovery key**. 
