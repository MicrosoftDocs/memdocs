...
*Custom compliant settings are in public preview.*
...
- The PowerShell script runs on a device to discover and report on the settings defined in the JSON. You upload scripts to Microsoft Endpoint Manager before you create a policy, and then select a single script when configuring the policy. The policy assigns the script to the device at the time the policy is evaluated. Each compliance policy supports a single script, and each script can detect multiple settings.

- The JSON file defines the settings you want to base your custom compliance on, and the acceptable values for those settings.  You’ll upload the file when you create a compliance policy that will include custom compliance settings.
...
During the workflow to create a compliance policy, on the Compliance setting page:
...
> [!NOTE]  
> When a Windows device receives a compliance policy with custom settings, the device runs an MSI that installs services that enable the client to download and run PowerShell scripts that are part of a compliance policy, and to upload compliance results. These services include:
>
> - StatelessSideCarGatewayService
> - StatelessSideCarService
> - StatefulSideCarService
> - StatelessComplianceService
> - StatelessComplianceCalculationService
>
> This service:
>
> - Checks for new or updated PowerShell scripts every eight hours.
> - Runs the discovery scripts every eight hours.
> - Runs downloaded scripts when a user selects Check Compliance on the device. However, the service doesn’t check for new or updated scripts when Check Compliance is run.
> - Doesn’t support push notifications to run on demand.
...
To see error related to the PowerShell script, add the following line to the end of the PowerShell script file: `return $hash | ConvertTo-Json -Compress`
...
Policies support the use of a single PowerShell script. However, each script supports checking for multipole compliance values.
