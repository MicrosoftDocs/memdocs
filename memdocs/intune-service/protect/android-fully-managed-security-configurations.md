...
| ----- | ----- | ----- | ----- |
| Device Health | Play Integrity Verdict | Check basic integrity | This setting requires devices to pass Google's Play Integrity API's basic integrity check. It verifies that the device is in a reasonably secure state, which means it is not rooted or running a custom ROM. |
...
| Applications | App auto-updates (work profile-level) | Wi-Fi only | Organizations should adjust this setting as necessary as data plan charges may occur if app updates occur over the cellular network. |
...
The level 2 settings include all the policy settings recommended for level 1. However, the settings listed below include only those settings that are added or changed. These settings can have a slightly higher impact to users or to applications. They enforce a level of security more appropriate for risks facing users with access to sensitive information on mobile devices.
...
| Applications | App auto-updates (work profile-level) | Always | Organizations should adjust this setting as necessary as data plan charges may occur if app updates occur over the cellular network. |
