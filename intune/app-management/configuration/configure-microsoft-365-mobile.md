...
To see the specific recommendations for each configuration level and the minimum apps that must be protected, review [Data protection framework using app protection policies](../protection/data-protection-framework.md).
...
For iOS/iPadOS devices running Office, you (as the admin) can set whether Microsoft 365 (Office) add-ins are enabled. These app settings can be deployed using an [app configuration policy](./configure-managed-ios.md) in Intune.
...
For more information about adding configuration keys, see [Add app configuration policies for managed iOS/iPadOS devices](./configure-managed-ios.md).
...
You can enable or disable offline caching when **Save As to Local Storage** is blocked by the [app protection policy](../protection/create-policy.md).
...
Admins can now enable or disable the Microsoft 365 Feed by configuring the following setting in the Intune admin center. To deploy this app setting, use an [app configuration policy](./overview.md) in Intune.
