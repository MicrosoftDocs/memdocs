...
6. The **Settings page** provides options that are displayed based on the app that you're configuring:
    ...
    - **General configuration settings** - For each general configuration setting that the app supports, type the **Name** and **Value**.
        ...
        Intune App SDK-enabled apps support configurations in key/value pairs. To learn more about which key-value configurations are supported, consult the documentation for each app. Note that you can use tokens that will be dynamically populated with data generated by the application. To delete a general configuration setting, choose the ellipsis (**…**) and select **Delete**. For more information, see [Configuration values for using tokens](./configure-managed-apps.md#configuration-values-for-using-tokens).
    ...
    - [Manage Microsoft Edge on iOS and Android with Intune](./configure-edge-ios-android.md)
    - [Manage collaboration experiences in Office for iOS and Android with Microsoft Intune](./configure-microsoft-365-mobile.md)
    ...
    - [Manage collaboration experiences in Teams for iOS and Android with Microsoft Intune](./configure-teams-mobile.md)
