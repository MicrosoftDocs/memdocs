...
| Set screen orientation | integer | 1 | Allows you to set the orientation of the home   screen to portrait mode, landscape mode or allow auto rotate. You can set the   orientation by entering values 1 (for portrait mode), 2 (for Landscape mode),   3 (for Autorotate). **Note:** On Android 16 and higher, the platform does not support this setting for devices with larger form factors, such as tablets, with smallest width 600 dp or larger.| ✔️ |
