...
| **Disabled account** | Specify an automated action if the Microsoft Entra account for the user is disabled.  Admin may specify only one Action.  There's no value to set for this setting.   *Actions* include: <br><ul><li>**Block access** - When we have confirmed the user has been disabled in Microsoft Entra ID, the app blocks access to work or school data.</li><li>**Wipe data**  - When we have confirmed the user has been disabled in Microsoft Entra ID, the app will perform a selective wipe of the users' account and data. </li></ul> **NOTE:** If user status can't be confirmed due to connectivity, authentication or any other reason, these actions (**Block access** and **Wipe data**) won't be enforced.  | *No default value* |
