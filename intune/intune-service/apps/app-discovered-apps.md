...
>- Software inventory collection for Discovered apps on Windows will have an initial delay up to 7 days for Windows (Win32 apps) applications on new devices that don't have a Microsoft Entra ID user signed in to the device. 
...
>- When using the iOS or macOS Restricted apps setting, Intune will collect but not store the full app inventory for personally owned devices.
