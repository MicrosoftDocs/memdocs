...
1. When opening Remote Help for the first time, you must allow Remote Help access to control and share your screen. Select on each of the required permissions to open Settings and ensure the permission is allowed for Microsoft Remote Help.
...
1. On the device, the user sees a prompt displaying a request to grant screen share or control of the device.
    ...
    b. If starting an unattended control session, the session will begin automatically after **30 seconds** if there's no response from the user.
...
1. When the session is ongoing
    ...
    c. During an unattended control session, the screen of the device you're connected to will blocked due to security and privacy reasons, and the user will be notified if they interact with it. If the user interacts with the blocked screen, they'll receive a notification letting them know that you're currently accessing the device. When the notification is shown, you and the end user won't be able to take any action for 30 seconds when this screen will close.

> [!NOTE]
> During an unattended control session, even with the screen of the device you're connected blocked to the end user, we recommend limiting your activities to nonsensitive operations.
