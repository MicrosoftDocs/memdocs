...
Today's mobile device management (MDM) strategies deal with a variety of phones, tablets, and PCs (iOS/iPadOS, Android, Windows, and macOS). If you need to be able to manage the device, which is commonly the case for corporate-owned devices, the first step is to [set up device enrollment](/intune/intune-service/fundamentals/deployment-guide-enrollment). You can also manage Windows PCs by enrolling them with Intune (MDM).
