...
6. Create a dynamic device group to automatically assign this profile to devices that fall within your rule parameters.
    ...
    1. In the **Value** column, enter the name of your enrollment policy.
...
| Show/hide   different apps to different users on a Shared iPad. | <ul><li>Assign all apps to the Microsoft Entra device group containing Shared iPads.</li><li>Create   allowed/blocked app lists to show/hide the apps and assign the policy to Microsoft Entra user groups.</li></ul> | When a user signs in on a Shared iPad, the user-assigned policy applies to show/hide the apps as configured in Microsoft Intune.<p><p>All apps assigned to the device show in Shared iPad temporary sessions. | You assign   Microsoft Outlook, Microsoft Teams, and Safari to a device group containing a Shared   iPad. Then you assign an allowed apps policy that only shows Teams to a user group containing users who only require Teams when using Shared iPad. You assign another allowed apps policy that shows Outlook, Teams and Safari to a user group containing managers who need access to all three apps when using Shared iPad. |
