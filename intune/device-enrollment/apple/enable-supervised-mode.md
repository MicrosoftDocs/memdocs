...
Apple iOS/iPadOS supervised mode gives administrators more options when managing Apple devices, making it useful for corporate-owned devices deployed at scale. For example, you can restrict AirDrop or prevent users from changing the name of the device. For a list of settings which require supervised mode, see [iOS device restriction settings in Intune](../../device-configuration/templates/ref-device-restrictions-apple.md).
...
In the [Microsoft Intune admin center](https://go.microsoft.com/fwlink/?linkid=2109431), you can turn on supervised mode for devices when you [create an Apple automated device enrollment policy](setup-automated-ios.md#create-an-apple-enrollment-policy). Under **Device Management Settings**, check the **Supervised** box.
