...
| `WRONG_USER` | Only one account per device can enroll an app with the MAM service. This result indicates that the account for whom this result was delivered (the second account) is targeted with MAM policy, but a different account is already enrolled. Because MAM policy can't be enforced for the second account, your app must not allow access to this account's data (possibly by removing the account from your app) unless/until enrollment for this account succeeds at a later time. Concurrent with delivering this `WRONG_USER` result, MAM prompts the end user with two options to remediate: (1) remove the existing enrolled account so that the second account can enroll, or (2) remove the second (attempted) account so the prompt stops appearing while the existing account remains enrolled. If the user chooses option (1), it will be possible to enroll the second account a short time later. If the user chooses option (2), MAM unregisters the second account locally and reports `UNENROLLMENT_SUCCEEDED` for that account via the standard enrollment-result notification; because the second account was never enrolled, no selective wipe is performed and no wipe notification is delivered. MAM also stops retrying enrollment for that account, so the prompt no longer reappears. As long as the second account remains registered (i.e., the user has not chosen option (2)), MAM retries enrollment periodically. |
...
![Intune Diagnostics UI](media/shared/intunediagnostics01.png) ![Intune App Info](media/shared/intuneappinfo01.png)
...
[Creating a test Android app protection policy]:android-phase-1.md#creating-a-test-android-app-protection-policy
...
[Understanding Company Portal logs]:android-appendix.md#understanding-company-portal-logs
[MDM and MAM enrollment]:android-appendix.md#mdm-and-mam-enrollment
...
[How to validate your app protection policy setup in Microsoft Intune]:../../app-management/protection/validate-policy-setup.md
