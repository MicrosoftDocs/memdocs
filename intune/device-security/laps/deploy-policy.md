...
> [!TIP]
> The Intune support for Windows LAPS is similar but distinct from Intune's implementation of macOS LAPS. For information about macOS LAPS in Intune, see [macOS local account configuration with LAPS](/intune/device-security/laps/setup-laps-macos).

## About Intune LAPS policy

Intune supports configuration of Windows LAPS on devices through the **Local admin password solution (Windows LAPS)** profile for endpoint security [account protection](../../device-configuration/endpoint-security/account-protection.md) policy.
