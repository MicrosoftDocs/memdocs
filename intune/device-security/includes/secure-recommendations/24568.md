...
- [Configure Platform SSO for macOS in Intune](/intune/device-configuration/settings-catalog/configure-platform-sso-macos) – *Step-by-step guidance for enabling Platform SSO on macOS devices.*
- [Single sign-on (SSO) overview and options for Apple devices in Microsoft Intune](/intune/device-configuration/enterprise-sso-plugin?pivots=macos) – *Overview of SSO options available for Apple platforms.*