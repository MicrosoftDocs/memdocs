...
## Setting values in compliance reports

Compliance reports may include a **Setting value** column that displays information reported directly by the device. This value provides additional context about why a device is marked as noncompliant for a given policy setting, and appears in scenarios such as custom compliance and Android app configuration reporting.

Because setting values are generated by device-side logic — such as an application or admin-supplied script — the service doesn't validate or enforce their content. The value reflects whatever the device or script reports and can vary in format and content.

Use setting values for context, but don't use them as the sole basis for administrative action. Confirm compliance status through trusted admin workflows and Intune reporting before acting on a device.

> [!IMPORTANT]
> Setting values are device-reported and not validated by Intune. The content may include free-form text, URLs, file paths, or other strings. Don't click links or open files referenced in a setting value unless you've independently verified the source. Avoid copying or sharing setting values that may contain user-identifying or environment-specific information.
