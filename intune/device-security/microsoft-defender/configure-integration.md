...
✅ **Conditional Access policies** that block noncompliant devices from corporate resources
...
1. **Open the Microsoft Defender portal:** From the Intune admin center, scroll to the bottom of the *Defender for Endpoint* page and select **Open the Defender Security Center** (or navigate directly to [security.microsoft.com](https://security.microsoft.com)).

   > [!TIP]
   > If already connected, the link reads: **Open the Defender for Endpoint admin console**.

   :::image type="content" source="./media/configure-integration/open-microsoft-defender.png" alt-text="Screen shot that shows the patch to open the Microsoft Defender Security Center.":::

1. **Enable the connection in Defender portal:** In the [Microsoft Defender portal](https://security.microsoft.com), go to **System** > **Settings** > **Endpoints** > **General** > **Advanced features**.

   :::image type="content" source="./media/configure-integration/defender-console-settings-endpoints.png" alt-text="Screen shot of the Defender console showing the path to Settings and then Endpoints.":::

   Locate **Intune connection**, toggle it to **On**, and then select **Save preferences**.

   :::image type="content" source="./media/configure-integration/intune-connection-toggle.png" alt-text="Screen shot of the Microsoft Intune connection setting.":::
...
1. **Validation:** Return to the Intune admin center. The **Connection status** should now show **Enabled** (it can take up to 15 minutes to update). You can review and adjust monitoring settings under **Endpoint security** > **Defender for Endpoint** if needed.
...
1. **Configure compliance policy evaluation:** Enable these options under **Compliance policy evaluation** for your supported platforms:
...
1. **Configure app protection policy evaluation:** Enable these options under **App protection policy evaluation** for mobile platforms:
   - **Connect Android devices to Defender for Endpoint**: **On**
   - **Connect iOS/iPadOS devices to Defender for Endpoint**: **On**
...
1. Select **Deploy preconfigured policy**.
   ...
   :::image type="content" source="./media/configure-integration/select-preconfigured-policy.jpg" alt-text="Screen shot that displays the path to the preconfigured policy option.":::
...
1. **Review and create**: Verify settings and select **Save**. The policy immediately starts deploying to all Windows devices.
...
1. **Basics:** Enter a descriptive name and optional description, and then select **Next**.
...
4. **Configuration settings:** Configure these options based on your requirements:
   ...
   :::image type="content" source="./media/configure-integration/automatic-package-configuration.png" alt-text="Screen shot of the configuration options for Endpoint Detection and Response.":::
...
1. **Configure settings**: Use Intune app configuration policies.
1. **Verify onboarding**: Check device appears in the Defender portal.
...
1. **Configure web protection**: Use [Microsoft Defender for Endpoint web protection](./configure-web-protection-android.md) policies for additional security.
1. **Verify onboarding**: Confirm device registration in the Defender portal.
...
1. **Configure supervision detection**: Set up supervised mode detection for enhanced features.
1. **Verify onboarding**: Check device registration in the Defender portal.
...
1. **Assignment**: Target **All Devices** or specific supervised device groups.

1. **Review + create**: Complete policy creation.
...
1. Review the onboarding status for all platforms.
...
1. **User assignment:**
   ...
   - **Exclude**: Exclude your organization's [emergency break-glass admin accounts](/entra/identity/role-based-access-control/security-emergency-access) to prevent lockout. If you use Microsoft Entra Connect or Microsoft Entra Connect Cloud Sync, also exclude the **Directory Synchronization Accounts** directory role.
...
1. **Enable policy:** Set **Enable policy** to **Report-only**, and then select **Create**. The policy is saved but doesn't block access yet.

1. **Review report-only results:** Wait 24 hours for sign-in data to accumulate, and then review the results:
...
1. **Validation:** Test with a noncompliant device to confirm access is properly blocked. Check **Microsoft Entra ID** > **Sign-ins** for policy enforcement logs.
...
1. **Validate protection**: Test with controlled scenarios to ensure policies work as expected.
1. **Expand protection**: Consider [vulnerability management](./remediate-vulnerabilities.md) for proactive threat remediation.
