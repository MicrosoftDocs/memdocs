...
5. On the **Deployment Settings** page, configure the following settings:
    ...
    - **Purpose**: Choose one of the following options:
        ...
        > [!NOTE]
        > If multiple users are signed in to the device, package and task sequence deployments appear in Software Center for the user in the active console session. If no user is signed in to the active console session, the deployments appear for the user with the lowest active session ID. When a user signs in, Windows starts a session for that user and identifies it with a unique session ID. To view the session ID and type for each signed-in user, open Task Manager, select the **Users** tab, right-click a column header, and then select **ID** and **Session**. The **Session** column shows the session type, such as **Console** for a local sign-in or an **RDP** session name for a remote connection. For more information, see [Remote Desktop Sessions](/windows/win32/termserv/terminal-services-sessions).
