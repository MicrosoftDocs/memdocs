...
8. Configure the following required settings:
    ...
    | **Token To User Mapping** > **Account Name** | `preferred_username` or `com.apple.PlatformSSO.AccountShortName` | Copy and paste this value in the setting. <br/><br/>This token specifies that the Microsoft Entra [`preferred_username`](/entra/identity-platform/id-token-claims-reference#payload-claims) attribute value is used for the macOS account's Account Name value. For PSSO during ADE, setting key value to `com.apple.PlatformSSO.AccountShortName` to use the UPN prefix. |
