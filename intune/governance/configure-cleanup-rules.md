...
Intune audit logs show the devices hidden by the device cleanup rules. For more information, see [Monitor audit logs in Intune](./monitor-audit-logs.md). In the logs, filter by **Activity name** > **Device set to be hidden from admin by Device Cleanup Rule [*Your Rule Name*]**.
