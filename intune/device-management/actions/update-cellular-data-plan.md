...
| --- | --- | --- | --- |
| Activate an eSIM on one or multiple devices | Android 15 or later | Android 15 or later | Android 15 or later |
...
Bulk activation uses the standard bulk device action workflow. The **Preview new device view** setting isn't required for bulk activation.
...
1. Enter the carrier activation server URL in Google's literal `$url$` format. Replace `url` with the carrier-provided URL, and keep the leading and trailing dollar signs. Select **Next**.
1. On the **Devices** page, select the supported Android Enterprise devices, and then select **Next**.
1. On the **Review + create** page, select **Create**.

Intune sends each activation request without first blocking it based on reported eSIM slot capacity. If Google can't complete an activation, Intune surfaces the error returned by Google.
