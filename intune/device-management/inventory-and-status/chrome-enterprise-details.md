...
1. In the Microsoft Intune admin center, [create a group](../../fundamentals/tenant-administration/add-groups.md#add-groups-to-intune) with the following details:
...
2. Create a scope tag for an Intune RBAC role. The scope tag determines the level of access for the Intune role. When you get to **Assignments**, include the dynamic device group you previously created. For more information, see [Use role-based access (RBAC) and scope tags for distributed IT](../../fundamentals/role-based-access-control/scope-tags.md#to-create-a-scope-tag).
