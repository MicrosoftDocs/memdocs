...
| Windows updates | Default Windows Update client policies are configured for users. With an Intune license, these settings can be edited. | Can be managed by using Intune. |
...
| [Per-user multi-factor authentication (MFA)](/azure/active-directory/authentication/howto-mfa-userstates) | Only MFA using [Microsoft Entra Conditional Access](/azure/active-directory/authentication/tutorial-enable-azure-mfa) is supported. Legacy per-user MFA isn't supported. | Legacy per-user MFA is supported for user connections to Microsoft Entra hybrid joined Cloud PCs.  It's not supported for user connections to Microsoft Entra joined Cloud PCs. |
