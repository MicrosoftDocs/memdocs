...
Starting with version **6.2504.2001.8** of the updated Intune Connector for Active Directory, the Intune Connector for Active Directory has switched to using WebView2, built on Microsoft Edge, instead of WebBrowser, built on Microsoft Internet Explorer. This change means that the Internet Explorer Enhanced Security Configuration setting in Windows Server no longer needs to be turned off. Make sure to run the Intune Connector for Active Directory installer version **6.2504.2001.8** or later to avoid issues with the Internet Explorer Enhanced Security Configuration setting.
