...
Device association is a feature of Windows Autopilot device preparation that binds a Windows device to your organization *before* the device enrolls in Microsoft Intune. It establishes a verifiable link between a physical device and your tenant by writing a tenant affinity marker into the device's UEFI firmware, making the device recognizable and authenticatable by Intune before enrollment begins. By establishing this trusted relationship early in the provisioning process, device association unlocks a streamlined out-of-box experience (OOBE) and strengthens the security of onboarding.
...
Device association involves three lifecycle operations:

| Operation | Performed by | Description |
|---|---|---|
| **Pre-associate** | Admin, in Intune | Temporarily stores the affinity between a device and your tenant in a central service, before the UEFI is stamped. |
| **Associate** | Automatic—triggered by the technician after pre-associating, or when the device connects to a network in OOBE | Writes a marker into the device's UEFI containing tenant affinity information. After this point, the UEFI marker is used to attest the device's affinity at enrollment. |
| **Remove association** | Admin, OEM vendor, or partner, by running a PowerShell script on the device | Deletes the UEFI marker used to verify the device's association, clearing the tenant affinity from the device. |
...
- **Apply device name template** - Name the device during enrollment by using a template. Names must be 15 characters or fewer and can contain letters, numbers, and hyphens, but can't contain only numbers. Use `%SERIAL%` to include the device serial number or `%RAND:x%` to include a random numeric string, where `x` is the number of digits.
