...
Device enrollment requires *Intune Administrator* or *Policy and Profile Manager* permissions. A custom Windows Autopilot device manager role can also be created by using [role-based access control (RBAC)](/intune/intune-service/fundamentals/role-based-access-control). Windows Autopilot device management requires only that all permissions under **Enrollment programs** are enabled, except for the four token management options.
