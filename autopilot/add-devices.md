...
- [Intune subscription](../memdocs/intune/fundamentals/licenses.md)
- [Windows automatic enrollment enabled](../memdocs/intune/enrollment/windows-enroll.md#enable-windows-automatic-enrollment)
...
Device enrollment requires *Intune Administrator* or *Policy and Profile Manager* permissions. You can also create a custom Autopilot device manager role by using [role-based access control](../memdocs/intune/fundamentals/role-based-access-control.md). Autopilot device management requires only that you enable all permissions under **Enrollment programs**, except for the four token management options.
...
1. If the devices are enrolled in Intune, [delete them from the Intune All devices pane](../memdocs/intune/remote-actions/devices-wipe.md#delete-devices-from-the-intune-admin-center).
