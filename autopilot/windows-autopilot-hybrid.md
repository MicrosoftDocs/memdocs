...
- The Intune Connector requires the [same endpoints as Intune](../memdocs/intune/fundamentals/intune-endpoints.md).
...
If you have a web proxy in your networking environment, ensure that the Intune Connector for Active Directory works properly by referring to [Work with existing on-premises proxy servers](../memdocs/intune/enrollment/autopilot-hybrid-connector-proxy.md).
...
10. On the **Scope tags** page, select [scope tags](../memdocs/intune/fundamentals/scope-tags.md) for this profile.
...
7. [Assign a device profile](../memdocs/intune/configuration/device-profile-assign.md#assign-a-policy-to-users-or-groups) to the same group used at the step [Create a device group](windows-autopilot-hybrid.md#create-a-device-group). Different groups can be used if there's a need to join devices to different domains or OUs.
...
After you configure Windows Autopilot, learn how to manage those devices. For more information, see [What is Microsoft Intune device management?](../memdocs/intune/remote-actions/device-management.md).
