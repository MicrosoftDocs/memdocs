...
- **Purpose** – From the drop-down list, choose one of the following options:
	...
	- **Required** The application is deployed automatically according to the configured schedule. If the application deployment status is not hidden, anyone using the application can track its deployment status and install the application before the deadline from Software Center.
...
3.  In the **Applications** list, select the application for which to delete the deployment.
...
When you receives required software from the **Snooze and remind me** setting, you can select from the following drop-down list of values:
...
Additionally, for a high-risk deployment, such as a task sequence that deploys an operating system, the user notification experience is now more intrusive. Instead of a transient taskbar notification, each time you are notified that critical software maintenance is required, a dialog box like the following displays on your computer:
