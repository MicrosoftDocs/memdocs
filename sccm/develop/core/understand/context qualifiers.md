---
title: "Configuration Manager Context Qualifiers"
ms.custom: ""
ms.date: "2016-09-20"
...
## Managed Code  
 When using the managed SMS Provider libraries, you use the <xref:Microsoft.ConfigurationManagement.ManagementProvider.ConnectionManagerBase.Context*> property to specify context qualifiers. For more information, see [How to Add a Configuration Manager Context Qualifier by Using Managed Code](../../../develop/core/understand/how-to-add-a-configuration-manager-context-qualifier-by-using-managed-code.md).  
  
## VBScript  
 When using VBScript, you use the [SWBemNamedValue](assetId:///SWBemNamedValue?qualifyHint=False&autoUpgrade=True) interface set to specify context qualifiers as a collection of named value objects. For more information, see [How to Add a Configuration Manager Context Qualifier by Using WMI](../../../develop/core/understand/how-to-add-a-configuration-manager-context-qualifier-by-using-wmi.md).  
...
|`InstanceCount`|Limits the number of instances returned from <xref:ExecQuery> and <xref:CreateInstanceEnum>.|  
...
## ApplicationName  
 The `ApplicationName` context qualifier is a string value that identifies the name of the application that made the call. You should specify `ApplicationName` for your application because it is used for auditing. If you do not supply the name of your application, a value of Unknown is used. You must supply the `ApplicationName` value when you call any of the raise status message methods, such as [SMS_StatusMessage](assetId:///SMS_StatusMessage?qualifyHint=False&autoUpgrade=True)::[RaiseErrorStatusMsg](assetId:///RaiseErrorStatusMsg?qualifyHint=False&autoUpgrade=True), or the call will fail.  
...
1.  Create assetId:///SWBemNamedValue?qualifyHint=False&autoUpgrade=True value set  
...
3.  Call the <xref:GetContextHandle> method to cache your qualifiers on the server. The SMS Provider caches the context object that you pass as a parameter of [ExecMethod](assetId:///ExecMethod?qualifyHint=False&autoUpgrade=True) when you call [GetContextHandle](assetId:///GetContextHandle?qualifyHint=False&autoUpgrade=True).  
  
4.  Remove all the qualifiers from your context object.  
  
5.  Add the `ContextHandle` qualifier and value to your context object.  
  
6.  Pass the context object on all calls to [IWbemServices](assetId:///IWbemServices?qualifyHint=False&autoUpgrade=True).  
  
 You must call the <xref:ClearContextHandle> method to remove your cached qualifiers before you exit your application. You can create as many `ContextHandle` values as you want, with each providing varying information for your application.  
...
## InstanceCount  
 The `InstanceCount` context qualifier is an integer value that is used to limit the number of instances returned from the <xref:ExecQuery> and <xref:CreateInstanceEnum> methods. You set `InstanceCount` equal to the maximum number of instances that you want returned from the query or enumerator. For example, setting `InstanceCount` to 10 returns, at most, 10 instances.  
  
## LimitToCollectionIDs  
 The `LimitToCollectionIDs` context qualifier is a string array that contains a list of `CollectionID` values. Currently, you can specify only one `CollectionID` value. You use this qualifier to limit the results of a resource query to the members of the named collection. A resource query is a query that includes classes derived from [SMS_Resource](assetId:///SMS_Resource?qualifyHint=False&autoUpgrade=True) or [SMS_Group](assetId:///SMS_Group?qualifyHint=False&autoUpgrade=True).  
