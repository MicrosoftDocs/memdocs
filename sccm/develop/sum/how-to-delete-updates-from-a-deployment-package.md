...
3.  Remove update content from the existing software updates management package by using the assetId:///RemoveContent?qualifyHint=False&autoUpgrade=True method.  

## Example  
 The following example method shows how to remove updates from a software updates deployment package by using the assetId:///SMS_SoftwareUpdatesPackage?qualifyHint=False&autoUpgrade=True class and the assetId:///RemoveContent?qualifyHint=False&autoUpgrade=True method.  

> [!IMPORTANT]
>  No VBScript example was included, as the assetId:///RemoveContent?qualifyHint=False&autoUpgrade=True method does not return from the method call on failure. This is a known issue and is being investigated.  
