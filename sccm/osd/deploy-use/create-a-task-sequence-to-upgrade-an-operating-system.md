...
- **Battery checks**: Add steps in this group to check whether the computer is using battery, or wired power. This action requires a custom script or utility to perform this check. For example: </br>Using WbemTest, connect to root\cimv2 namespace and run wmiquery `Select Batterystatus From Win32_Battery where batterystatus != 2`. If any results returned then it is connected to battery power else connected to ACPower`</br>  
