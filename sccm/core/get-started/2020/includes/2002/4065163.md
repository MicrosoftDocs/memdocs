...
1. When the synchronization completes, decline any Office 365 updates that you don't want to deploy with Configuration Manager. You don't need to approve Office 365 updates in order for them to be downloaded.  
   ...
   - The OfflineUpdateExporter tool does does the download of Office 365 updates for you. Other products will still need to be approved for download if you are exporting updates for them.
...
1. At the command prompt on the internet connected WSUS server, navigate to the folder that contains WsusUtil.exe. By default, the tool is located at %*ProgramFiles*%\Update Services\Tools. For example, if the tool is located in the default location, type **cd %ProgramFiles%\Update Services\Tools**.
...
1. Once the import is complete, configure a site control property on the disconnected top-level Configuration Manager site server to point to the content for Office 365. Run the [O365OflBaseUrlConfigured.ps1 PowerShell script](#bkmk_o365_script) on the server . Change `"D:\Office365updates\content"` to 
