...
When a device runs a task sequence and needs to acquire content, it now uses boundary group behaviors similar to the Configuration Manager client. The task sequence uses the following behaviors when trying to acquire content:  
...
These settings are enabled by default, and available once clients update to the latest version. 
