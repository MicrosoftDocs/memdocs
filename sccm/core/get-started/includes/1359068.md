...
CMPivot now includes basic support for the Log Analytics [render operator](https://docs.loganalytics.io/docs/Language-Reference/Tabular-operators/render-operator). This support includes the following types:  
...
For example, select the count of devices with a Failure status. See the specific error message, and create a collection of these devices. If the error is that a specific cmdlet isn't recognized, use this collection to deploy an update to Windows PowerShell.  