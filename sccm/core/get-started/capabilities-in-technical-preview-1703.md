...
Upgrade Readiness data is retrieved from the Windows Analytics service. In order for Windows Analytics to be able to provide data related to computers in your environment, the client computers must be configured with a Commercial ID key in order to associate the data that the device reports to the Windows telemetry service with the OMS workspace that hosts your organization's Windows Analytics data. Additionally, client computers must be configured to report Windows telemetry at the prerequisite telemetry level for the Windows Analytics solutions that you want to leverage.
