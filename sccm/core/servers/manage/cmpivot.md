...
- (5) The query pane is where you build or type a query to run on clients in the collection.  
    ...
    - The query pane also provides the following options:  
        ...
        - Create a collection <!--QUESTION is this query or direct membership?  -->
...
<!--DO WE HAVE THE SAMPLE SCRIPT TO SHARE HERE?-->
...
<!--NEED TO REVISE THIS FOR A PROPER E2E SCENARIO-->

To be proactive with operational maintenance, once a week you run CMPivot against a collection of servers that you manage, and select the **AppCrash** entity. One device returns seven results for sqlsqm.exe with a timestamp about 03:00 every day. You select the file name in one of the rows, right-click and select **Bing It**. 

Your helpdesk is reporting an increase in user calls about the application sp2wp.exe. You start CMPivot against the **All Systems** collection, switch to the **Query** tab, expand the **AppCrash** entity, and select the **FileName** attribute. In the query pane, you type the file name to complete the query:  

`AppCrash | where (FileName == 'sp2wp.exe')`

You click Run Query, and CMPivot quickly returns data from active devices on which this app has crashed. As there is more than one report from each machine, you right-click the Device column and select **Group by Device**. You right-click the FileName column and select the option to **Bing It**. Browsing the search results in the web browser, you find a Microsoft support article for this issue with more information and resolution. 
