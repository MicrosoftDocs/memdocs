...
-   The client queries Active Directory Domain Services (AD DS) for published management points. To be identified from AD DS, the management point must be from the client's assigned site, and be of the same product version as the client.  
...
- You can add each server that hosts a management point to the DnsAdmins group to ensure those management points have permissions to modify their records.  
