...
> [!IMPORTANT]
> Although a pull-distribution point supports communications over HTTP and HTTPS, when you use the Configuration Manager, you can only specify source distribution points that are configured for HTTP. You can use the Configuration Manager SDK to specify a source distribution point that is configured for HTTPS.  
...
-   Only distribution points that support HTTP can be specified as a source distribution points when you use the Configuration Manager.  
...
-   This framework is installed by the **Pulldp.msi** when you configure the distribution point to be a pull-distribution point and does not require that the Configuration Manager client be installed.  
