...
Configuration Manager groups data that replicates by database replication into different replication groups. Each replication group has a separate, fixed replication schedule. The schedule determines how frequently changes to the data in the group are replicated to other sites.  

For example, a change to a role-based administration configuration replicates quickly to other sites. This action makes sure that the other site can quickly enforce these changes. Meanwhile, a lower-priority configuration change, such as a request to install a new secondary site, replicates with less urgency. It can take several minutes for a new site request to reach the destination primary site.  
...
Control when specific traffic traverses the network.  

### Distributed views

Change settings for replication links by which requests that are made at a central administration site (CAS) for selected site data can access that site data directly from the database at a child primary site.  

### Schedules

Specify when a replication link is used, and when different types of site data replicate.  

### Summarization

Change settings for data summarization about network traffic that traverses replication links. Summarization occurs every 15 minutes, by default, and is used in reports for database replication.  

### Database replication thresholds

Define when the site reports links as degraded or failed. You can also configure when Configuration Manager raises alerts about replication links that have a degraded or failed status.  
...
A third data type, *local data*, doesn't replicate to other sites. Local data is information that other sites don't require.  

### Global data

Global data refers to administrator-created objects that replicate to all sites throughout the hierarchy. Secondary sites receive only a subset of global data, as global proxy data. Administrators can create global data at the CAS and primary sites.

Some examples of global data include:
...
Site data refers to operational information created by Configuration Manager primary sites and their assigned clients. Site data replicates to the CAS but not to other primary sites.

Site data is only viewable at the CAS and at the primary site where the data originates. You can only modify site data at the primary site where you created it.  

Some examples of site data include:
...
All site data replicates to the CAS. The CAS performs administration and reporting for the entire site hierarchy.  
...
To control the transfer of data across the replication link, change settings for each link. Each replication link supports separate configurations.

Database replication links include the following controls:  
...
> [!TIP]  
> You can edit database replication links from the **Database Replication** node in either workspace. However, when you use the **Database Replication** node in the **Monitoring** workspace, you can also view the status of database replication for the links. You can also access the Replication Link Analyzer (RLA) tool to help you investigate problems with database replication.  
...
Through distributed views, when a central administration site (CAS) requests site data, it accesses that data directly from the child primary site database. This direct access replaces the need to replicate site data from the primary site to the CAS. Because each replication link is independent from other replication links, you can use distributed views on the replication links that you choose. You can't use distributed views between a primary site and a secondary site.  
...
- Reduce the disk space used by the database at the CAS

Consider using distributed views when a primary site is closely located to the CAS on the network, and the two sites are always on and connected. Distributed views replace the replication of the selected data between the sites with direct connections between the SQL servers at each site. A direct connection is made each time a request for this data is made at the CAS.

You might enable distributed views for the following data requests:

- Run reports or queries
- View information in Resource Explorer
- Collection evaluation for collections that include rules that are based on the site data

By default, distributed views are turned off for each replication link. When you turn on distributed views for a replication link, you select site data that won't replicate to the CAS across that link. The CAS accesses this data directly from the database of the child primary site that shares the link. 

Configure the following types of site data for distributed views:  
...
Distributed views are invisible to an administrative user who views data in the Configuration Manager console or in reports. When the site requests data that you enabled for distributed views, the CAS site database server retrieves the information directly from the child primary site database.

For example, you use a Configuration Manager console connected to the CAS. You request information about hardware inventory from two sites: ABC and XYZ. You only enabled hardware inventory for distributed views at site ABC. The CAS retrieves inventory information for XYZ clients from its own database. The CAS retrieves inventory information for ABC clients directly from the database at site ABC. This information appears in the Configuration Manager console or in a report without identifying the source.  
...
- The CAS can have only one instance of the SMS Provider. Install that single instance on the site database server. This configuration supports Kerberos authentication for the CAS database server to access the primary site database server. There are no limitations on the SMS Provider at the child primary site.

- You can only install one reporting services point at the CAS. Install SQL Server Reporting Services on the site database server. This configuration supports Kerberos authentication, which enables the CAS database server to access the primary site database server.

- You can't host the site database on a SQL Server cluster.

- You can't host the site database on a SQL Server Always On availability group.
...
## <a name="BKMK_SummarizeDBReplication"></a> Summarization of replication traffic  

Each site periodically summarizes data about the network traffic that traverses database replication links for the site. The site uses summarized data in reports for database replication. Both sites on a replication link summarize the network traffic that traverses the replication link. The SQL Server that hosts the site database does the summarization of data. After data is summarized, the information replicates to other sites as global data.  

By default, summarization occurs every 15 minutes. To modify the frequency of summarization for network traffic, in the properties of the database replication link, edit the **Summarization interval**. The frequency of summarization affects the information that you view in reports about database replication. Choose an interval between 5 minutes and 60 minutes. When you increase the frequency of summarization, you increase the processing load on the SQL Server at each site on the replication link.  
...
To fine-tune when Configuration Manager reports a replication link as degraded or failed, specify custom values. You can more accurately monitor replication health across your database replication links by adjusting when Configuration Manager reports each status for your links.  

One or more replication groups can fail to replicate while other replication groups continue to successfully replicate. Plan to review the replication status of a replication link when it first reports as degraded.
