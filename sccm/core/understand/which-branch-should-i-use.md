...
The current branch is updated several times a year with new features. Each update version is supported for one year after its release. Update to a newer version of the current branch on or before expiration of that one-year period. Updates to newer versions are available as in-console updates.
