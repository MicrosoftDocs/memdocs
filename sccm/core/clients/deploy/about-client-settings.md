...
Select **Browse** to select an image to appear in Software Center. The logo must be a JPEG, PNG, or BMP of 400 x 100 pixels, with a maximum size of 750 KB.<!--SMS.503731 space in filename, noticed BMP missing as filetype-->  The logo file name should not contain spaces. 
        
### Hide unapproved applications in Software Center
Starting in Configuration Manager version 1802,<!--1355146-->  when this option is enabled, user available applications that require approval are hidden in Software Center.  
    
### Hide installed applications in Software Center
Starting in Configuration Manager version 1802,<!--1357592--> applications that are already installed will no longer show in the Applications tab when this option is enabled.    
