...
- Devices that don't have associated users (typically, when you have iOS Device Enrollment Program or bulk enrollment scenarios), and you perform an MDM reset, then those devices are not migrated to the new MDM authority. For those devices, you need to call support for assistance to move the devices to the new MDM authority.
