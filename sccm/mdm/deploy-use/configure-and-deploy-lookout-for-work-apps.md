...
1. Make sure iOS management is set up on your devices. For instruction on how to set up your device for iOS management, see [Set up iOS and Mac device management](/sccm/mdm/deploy-use/enroll-hybrid-ios-mac).  
...
3. Enable Azure Active Directory (Azure AD) authentication for the iOS users.
  ...
  ![screenshot of the add apps dialog showing the native client app option](media/aad-add-app-reg.png)
   ...
   3. Replace `com.lookout.enterprise.yourcompanyname` with the customer bundle ID you selected when you signed the .ipa file.  
   4.  Add the following additional redirect URI: `<companyportal://code/>` followed by a URL-encoded version of your original redirect URI.
