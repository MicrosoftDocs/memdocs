...
|**Idle time before device is locked**|Choose the amount of time before the device will be locked if it is not being used.|
|**Password quality**|Choose the password complexity level required and also whether biometric devices can be used.|  
...
|-------------|----------------------|  
|**Noncompliant apps list**|Choose this option if you want to specify a list of apps that will be reported as noncompliant if installed by users.|  
|**Compliant apps list**|Choose this option if you want to specify a list of apps that users are allowed to install. Any other installed apps will be reported as noncompliant.|  
...
### Kiosk mode (Samsung KNOX only)  
 You can use kiosk mode to lock a device to only allow certain features to work. For example, you can allow a device to only run one managed app that you specify, or you can disable the volume buttons on a device. These settings might be used for a demonstration model of a device, or a device that is dedicated to performing only one function, such as a point of sale device.  
